...
export function fuzzyAtLocation(current, destination, tolerance) {
  return Math.abs(current.x - destination.x) < tolerance
    && Math.abs(current.y - destination.y) < tolerance;
}

export function distanceBetweenBounds(b1, b2) {
  // Is only finding distance between centers, should actually be between edges
  return Phaser.Math.Distance.Between(
    b1.centerX, b1.centerY,
    b2.centerX, b2.centerY,
  );
}
